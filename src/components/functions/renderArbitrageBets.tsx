          ...
          if (b1.key !== b2.key) {
            ...
            if (m1 && m2) {
              ...
              if (o1 && o2 && checkArbitrage(o1.price, o2.price)) {
                const profit = calculateProfitPercent(o1.price, o2.price, 1000);
                const jsx = (
                  <Box
  ...
  const renderSpreadArbitrage = () => {
    games.forEach((game) => {
      const altGame = altGames.find(
        (g) => g.home_team === game.home_team && g.away_team === game.away_team
      );
        ...
        game.bookmakers.forEach((b2) => {
          if (b1.key !== b2.key) {
            const m1Combined = combineMarkets(
              b1.key,
              [game, altGame].filter(Boolean) as Game[],
