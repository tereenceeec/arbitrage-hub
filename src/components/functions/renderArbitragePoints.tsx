...
const extractPlayerPoints = (markets?: Market[]) =>
  markets?.find((m) => m.key === 'player_points');
...
export const renderArbitragePoints = (games: Game[]): JSX.Element => {
  ...
  games.forEach((game) => {
    ...
    game.bookmakers.forEach((bookmaker) => {
      const pointsMarket = extractPlayerPoints(bookmaker.markets);
      if (!pointsMarket) return;

      pointsMarket.outcomes.forEach((outcome) => {
